...
class ParseTxt {
    ...
    public function parser($content, $lines) {
        ...
        foreach ($lines as $line) {
            if (preg_match(
                ...
                '/^(?P<dia>(LU|MA|MI|JU|VI|SA)) ' .
                '(?P<inicio>\d{3,4})-(?P<final>\d{3,4})\((?P<aula>.*)\)$/',
                    $line, $output)) {
                $dia = $output['dia'];
            ...
            } else if (preg_match('/^([A-Z¥ \.\']+|Por Designar ...)$/',
                $line, $output)) {
    ...
    public function transform() {
        ...
        file_put_contents(substr($dir, 0, -1) . '.json',
            json_encode($_carreras));
    }
}
